...
#Reading ATC list from excel spreadsheet - used for Exams 28 to 32
...
#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of these types
thyro_filtered30 <- cod_30 %>% filter_all(any_vars(. %in% thyro_atc_list))
#Constructing the output data frame
atc_ID_30 <- unique(cod_30$id)
med_found30 <- data.frame(atc_ID_30)
#Formatting ATC rows into 0 or 1 notation for absent or present
med_found30['thyroid_med_core30'] <- as.integer(med_found30$atc_ID_30 %in% thyro_filtered30$id)
#prepare for upcoming merge
colnames(med_found30)[1] <- "ID"
#replacement of missing values for upcoming merge
all_30 <- med_found30 %>% mutate(across(where(is.numeric), ~ replace_na(as.character(.x), ".")))
...
#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of these types
thyro_filtered31 <- cod_31 %>% filter_all(any_vars(. %in% thyro_atc_list))
#Constructing the output data frame
atc_ID_31 <- unique(cod_31$id)
med_found31 <- data.frame(atc_ID_31)
#Formatting ATC rows into 0 or 1 notation for absent or present
med_found31['thyroid_med_core31'] <- as.integer(med_found31$atc_ID_31 %in% thyro_filtered31$id)
#prepare for upcoming merge
colnames(med_found31)[1] <- "ID"
#replacement of missing values for upcoming merge
all_31 <- med_found31 %>% mutate(across(where(is.numeric), ~ replace_na(as.character(.x), ".")))

#Exam 32
#medications from exam 32
meds_32 <- read_sas("vr_meds_ex32_0_0880.sas7bdat")
cod_32 <- meds_32[,c("id", "atc_cod1", "atc_cod2", "atc_cod3", "atc_cod4")]

#Select the rows of individual ATC data containing the specific ATC numbers in the lists for drugs of these types
thyro_filtered32 <- cod_32 %>% filter_all(any_vars(. %in% thyro_atc_list))
#Constructing the output data frame
atc_ID_32 <- unique(cod_32$id)
med_found32 <- data.frame(atc_ID_32)
#Formatting ATC rows into 0 or 1 notation for absent or present
med_found32['thyroid_med_core32'] <- as.integer(med_found32$atc_ID_32 %in% thyro_filtered32$id)
#prepare for upcoming merge
colnames(med_found32)[1] <- "ID"
#replacement of missing values for upcoming merge
all_32 <- med_found32 %>% mutate(across(where(is.numeric), ~ replace_na(as.character(.x), ".")))
...
gen1_select_list <- list(select_07, select_08, select_09, select_10, select_11, select_12, select_13,
                         ...
                         all_28, all_29, all_30, all_31, all_32)
